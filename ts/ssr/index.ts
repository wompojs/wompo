...
const ssRenderComponent = (
	...
	for (let i = 0; i < template.parts.length; i++) {
		...
		if (indexOfSelfClosing !== -1 && indexOfSelfClosing < part.indexOf('<')) pendingClosing.pop();
		if (part.endsWith('<')) pendingClosing.push(value);
		if (part.endsWith('</')) templateValues.push(pendingClosing.pop());
		if (i < template.values.length) templateValues.push(value);
	}
