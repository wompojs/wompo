...
export const useLayoutEffect = (
	...
	if (!component._$hooks.hasOwnProperty(hookIndex)) {
		...
		const effectHook = component._$hooks[hookIndex] as EffectHook;
		if (dependencies !== null) {
			for (let i = 0; i < dependencies.length; i++) {
...
export const useCallback = (callbackFn: (...args: any[]) => any, dependencies: any[] = []) => {
	...
	if (!component._$hooks.hasOwnProperty(hookIndex)) {
		...
		const callbackHook = component._$hooks[hookIndex] as CallbackHook;
		for (let i = 0; i < dependencies.length; i++) {
	...
	const callback = component._$hooks[hookIndex] as CallbackHook;
...
const executeUseAsyncCallback = <S>(
	...
	(component._$hooks[hookIndex] as AsyncHook<S>).value = null;
	const promise = callback();
	promise
		.then((data) => {
			component.requestRender();
			suspense.removeSuspense(component);
			(component._$hooks[hookIndex] as AsyncHook<S>).value = data;
		})
...
export const useAsync = <S>(callback: () => Promise<S>, dependencies: any[]): null | S => {
	...
	if (!component._$hooks.hasOwnProperty(hookIndex)) {
		...
		const oldAsync = component._$hooks[hookIndex] as AsyncHook<S>;
		let newCall = false;
		for (let i = 0; i < dependencies.length; i++) {
	...
	return (component._$hooks[hookIndex] as AsyncHook<S>).value;
};
...
export type LazyCallbackResult = Promise<{ default: WompComponent }>;
...
interface SuspenseInstance extends WompElement {
...
const findSuspense = (startNode: Node): SuspenseInstance | null => {
	...
	while (
		suspense &&
		suspense.nodeName !== (Suspense as WompComponent).componentName.toUpperCase()
	) {
